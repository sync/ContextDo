...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
  ...
  self.expirationDate = expirationDate;
	if ([self.sessionDelegate respondsToSelector:@selector(fbDidLoginWithPermissions:)]) {
		NSString *scope = [_loginDialog.params valueForKey:@"scope"];
		NSArray *permissions = [scope componentsSeparatedByString:@","];
		[_sessionDelegate fbDidLoginWithPermissions:permissions];
  }
...
- (void) fbDialogNotLogin:(BOOL)cancelled {
	if ([self.sessionDelegate respondsToSelector:@selector(fbDidNotLogin:permissions:)]) {
		NSString *scope = [_loginDialog.params valueForKey:@"scope"];
		NSArray *permissions = [scope componentsSeparatedByString:@","];
		[_sessionDelegate fbDidNotLogin:cancelled permissions:permissions];
  }
}
